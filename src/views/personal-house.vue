<template>
  <div class="per-house">
    <nav-top :nav-list="navList" activeName="个人库"></nav-top>
    <div class="double">
      <div class="left">
        <div class="sel-top">
          <el-cascader
        ...
        <div class="kuang" :class="show3?'aa':'bb'">
          ...
          <div class="top">
            ...
            <span class="nbor-btn" style="margin-right:20px;" @click="delPower()">删除</span>
          </div>
          <div class="scr">
            <!-- @cell-dblclick="db" -->
            <el-table
              height="100%"
              @row-click="getPerSingPower"
              class="my-table"
              :data="powers"
              tooltip-effect="dark"
            ...
            >
              ...
              <el-table-column prop="airDensity" label="空气密度(kg/m3)">
  ...
  methods: {
    ...
    putPower() {
      this.projectHttps
        ...
        .then(() => {
          this.keepMa = "";
          this.getPerPowerList(this.typeId);
        })
    ...
    uploadWtg() {
      if (this.typeId) {
        this.projectHttps
          .uploadWtg(this.typeId, this.file, this.form1.turbulence)
          .then(res => {
            this.getPerPowerList(this.typeId);
            // this.powers = res;
            this.dialogShow = false;
    ...
    getPerSingPower(row, column, event) {
      this.values = [];
      row && ((this.ids[0] = row.id), (this.ids[1] = row.name));
      this.projectHttps
        .getPerSingPower(this.ids[0])
        .then(res => {
          this.windSet.yData = [];
          if (res.length == 1) {
            this.power = res[0];

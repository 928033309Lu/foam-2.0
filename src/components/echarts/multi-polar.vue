<template>
  <div class="multi-polar">
    ...
    <div class="my-legend">
      <ul>
        <li v-if="legend && legend.length>0">风速 m/s</li>
        <li @click="changeLegend(i)" v-for="(item,i) in legend" :class="item.checked ? 'checked' : 'no-checked'"><i class="round-rect" :style="'background-color:'+colors[i]"></i>{{item.name}}</li>
      </ul>
    </div>
  </div>
</template>
...
//        this.echart.height = `${(1/rowNum*100-1)}%`
        ...
        if(newW === this.echart.width && this.echart.height === newH){
          ...
          this.checkDevice.forEach((d, k) => {
            ...
            if (k == 0) {
              this.legend = v[vm.windSet.legendName].map((leg) => {
                return {
                  name: leg,
                  checked: true
                }
